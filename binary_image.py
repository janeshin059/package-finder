import requests
import json
import sys

# ==============================================================================
# 설정: API 키와 URL을 여기에 입력하세요.
# ==============================================================================
PRISMA_CLOUD_BEARER_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..A25BZB5J0kj78Fgv_KnxGb0_MfddQHXSn3R4xD9qgzk"
PRISMA_CLOUD_URL = "asia-southeast1.cloud.twistlock.com/aws-singapore-961149788"
PRISMA_CLOUD_VERSION = "v34.02"

def get_binary_info_by_name(binary_name):
    """
    모든 이미지 보고서를 검색하여 특정 바이너리 이름의 정보를 찾습니다.

    Args:
        binary_name (str): 검색할 바이너리의 이름.

    Returns:
        list: 검색된 바이너리 정보가 담긴 딕셔너리 리스트.
    """
    found_binaries = []
    headers = {
        "Authorization": f"Bearer {PRISMA_CLOUD_BEARER_TOKEN}",
        "Accept": "application/json"
    }

    print(f"'{binary_name}' 바이너리 정보를 검색 중...")

    try:
        url = f"https://{PRISMA_CLOUD_URL}/api/{PRISMA_CLOUD_VERSION}/images"
        
        response = requests.get(url, headers=headers, verify=False)
        response.raise_for_status()
        
        reports = response.json()
        
        if not reports:
            print("더 이상 이미지 보고서가 없습니다.", file=sys.stderr)
            return []
      
        for report in reports:
            binaries_in_image = report.get('binaries')
            if binaries_in_image:
                for binary in binaries_in_image:
                    if binary.get('name') == binary_name:
                        found_binaries.append({
                            "name": binary.get("name"),
                            "path": binary.get("path"),
                            "md5": binary.get("md5"),
                            "version": binary.get("version"),
                            "image_id": report.get("id"),
                            "image_name": report.get("repoTag", {}).get("repo"),
                            "registry": report.get("repoTag", {}).get("registry")
                        })

    except requests.exceptions.RequestException as e:
        print(f"API 호출 중 오류 발생: {e}", file=sys.stderr)
        return []

    return found_binaries

def main():
    """
    메인 스크립트 실행 함수.
    """
    if "YOUR_" in PRISMA_CLOUD_BEARER_TOKEN or "YOUR_" in PRISMA_CLOUD_URL:
        print("에러: Prisma Cloud Bearer 토큰과 URL을 올바르게 입력해주세요.", file=sys.stderr)
        sys.exit(1)

    search_binary_name = input("검색할 바이너리 이름을 입력하세요: ")
    if not search_binary_name:
        print("바이너리 이름이 입력되지 않았습니다. 스크립트를 종료합니다.", file=sys.stderr)
        sys.exit(1)

    binaries = get_binary_info_by_name(search_binary_name)

    if binaries:
        print(f"\n '{search_binary_name}' 바이너리 정보:")
        for binary in binaries:
            print("-" * 20)
            print(f"  이름: {binary.get('name', 'N/A')}")
            print(f"  경로: {binary.get('path', 'N/A')}")
            print(f"  MD5: {binary.get('md5', 'N/A')}")
            print(f"  버전: {binary.get('version', 'N/A')}")
            print(f"  포함된 이미지 ID: {binary.get('image_id', 'N/A')}")
            print(f"  포함된 이미지 이름: {binary.get('image_name', 'N/A')}")
            print(f"  레지스트리: {binary.get('registry', 'N/A')}")
    else:
        print(f"\n '{search_binary_name}' 바이너리 정보를 찾을 수 없습니다.")

if __name__ == "__main__":
    main()